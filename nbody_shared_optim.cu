...
__global__ void bodyForce(Body *p, float dt, int n)
{
    ...
    if (i < n)
    {
        ...
        //__shared__ float3 spos[BLOCK_SIZE];   //块内共享
        ...
        for (int block_num = start_block; block_num < cycle_times; block_num += BLOCK_STRIDE)
        {
            ...
            shared_x[threadIdx.x] = temp.x;
            shared_y[threadIdx.x] = temp.y;
            shared_z[threadIdx.x] = temp.z;
            // 块内同步，防止spos提前被读取
            __syncthreads();
...
#pragma unroll
            ...
            {
                dx = shared_x[j] - ptemp.x;
                dy = shared_y[j] - ptemp.y;
                dz = shared_z[j] - ptemp.z;
